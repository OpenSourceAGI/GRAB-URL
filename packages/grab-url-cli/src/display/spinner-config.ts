...
import * as spinnerData from "../../../loading-animations/src/cli/loading-animations-emojis.js";
...
export function getSpinnerFrames(spinnerType: string): string[] {
  ...
  return getFrames((spinnerData as any).dots);
}
 ...
 */
export function calculateBarSize(
  spinnerFrame: string,
  baseBarSize = 20,
): number {
  ...
  const available =
    terminalWidth - otherElements - filenameWidth - spinnerWidth;
