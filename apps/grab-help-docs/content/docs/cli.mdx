...
The CLI ships as its own package, **`grab-url-cli`** — separate from the `grab-url`
library so importing `grab()` in an app stays a ~5 kB dependency with no install scripts.

```bash
## install for CLI testing of your APIs
npm i -g grab-url-cli

# use "grab" (or "g") if installed, or "npx grab-url-cli" if not
...
npx grab-url-cli https://example.com/a.zip https://example.com/b.zip -d ./downloads
```
...
npx grab-url-cli sftp://host:2222/srv/db.tar.zst --user alice --password hunter2 -o backup.tar.zst

# Pin the host key — the transfer aborts if the server presents a different one
npx grab-url-cli sftp://host/srv/db.tar.zst --user alice --password hunter2 \
...
npx grab-url-cli ./ubuntu.torrent --aria2-args "--max-overall-download-limit=2M --enable-dht=false"
```
