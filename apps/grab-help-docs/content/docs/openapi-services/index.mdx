...
The SDK, the MCP server and the API reference all read the same file, so they
cannot drift from each other — there is only one spec to change.

```bash
npx api2client ./openapi.json ./src/client            # typed SDK, sent by grab
npx api2ai ./openapi.json ./mcp-server --name my-api  # MCP server for agents
```

For the third, render the spec with any OpenAPI viewer —
[Scalar](https://scalar.com)'s `createApiReference` mounts it into a page in one
call:
...
createApiReference(document.getElementById("api-reference")!, { content: spec });
```

The example below does all three at once, and includes an
[`mcp-use`](https://mcp-use.com) client that connects to the generated server,
lists its tools and calls one.
