...
`grab.mock` keys are request paths, so any SDK endpoint can be stubbed without touching the network:
...
Requires `grab-url` ≥ 1.6.23, which added the `onRawResponse` hook this client uses to report status codes, headers and error payloads.
