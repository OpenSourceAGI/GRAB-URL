...
| A file to download, or an SFTP / torrent / magnet transfer, from a terminal          | `npx grab-url-cli <url>` — the [CLI](#cli)                            |
| A web page to keep — its article, citation, transcript and video — from a terminal   | `npx grab-url-cli <url> --page` — [page archives](#page-archives)      |
...
Requires Node 18+ and `grab-url` ≥ 3.0.0 (`api2client` ≥ 2 depends on it). The generated SDK
imports `grab-url`, which since 3.0 is the ~5 kB slim build — an OpenAPI response never needs
the DOM parser or the unzipper, so the whole generated transport layer stays under 100 kB.
...
`import grab from "grab-url"` is the slim build — ~5 kB gzipped, no ZIP or DOM
post-processing, which is what almost every app wants. When a response needs auto-unzipping
(`unzip`) or HTML parsed into a DOM (`parseDOM`), import `grab-url/full` instead; it adds
linkedom and the archive extractor, both behind lazy imports that only load when a response
actually triggers them.

```ts
import grab from "grab-url";        // default — slim
import grab from "grab-url/full";   // only if you use `unzip` or `parseDOM`
```
...
Node, the browser, Cloudflare Workers and the CLI, and creates ZIPs as well as reading them:

```ts
import { compress, extract, extractStream } from "archiver-web";

// extract takes the archive's bytes, not a URL — grab or fetch them first.
const bytes = await grab("https://github.com/user/repo/archive/main.zip", { unzip: false });
const files = await extract({
  archiveBuffer: await bytes.arrayBuffer(),
  folderPath: "src/",
...
const archive = await compress({
  ...
  outputName: "out.zip",
}); // { blob, mime, downloadName }
```

`extractStream({ stream, onFile })` is the incremental form, and what `unzip` uses internally.
...
`grab-url-cli` is a command-line request runner and transfer tool — useful for exercising an
...
It is a **separate package** from the `grab-url` library: installing the library gives you a
~5 kB import and no install scripts, while the CLI brings its terminal dependencies and a
yt-dlp download with it.

```bash
npm i -g grab-url-cli      # then `grab`, `g`, or `grab-url`; or just `npx grab-url-cli`
```
...
npx grab-url-cli https://api.example.com/data --no-save     # print to stdout, write nothing
...
npx grab-url-cli https://youtu.be/dQw4w9WgXcQ --page -d ./archive --lang es,en

# Background transfers
npx grab-url-cli https://example.com/big.iso --background    # detach at once
npx grab-url-cli --jobs                                      # list what is still running
```
...
```bash
npx grab-url-cli https://example.com/article --page              # ./<Page Title>/
npx grab-url-cli https://youtu.be/dQw4w9WgXcQ --page -d ./archive
npx grab-url-cli https://example.com/article --page --no-video   # text only
npx grab-url-cli https://example.com/talk --page --video-format bestaudio
npx grab-url-cli https://youtu.be/x --page --lang es,en          # transcript language order
```
...
| --------------------------------- | --------------------------------------------------------------------- |
| `grab-url`                        | **The default** — `grab()`, `log()`, the globals, DevTools, cache, mocks, ~5 kB gzipped |
| `grab-url/full`                   | The same plus ZIP extraction and DOM parsing (`unzip`, `parseDOM`)   |
| `grab-url/slim`                   | Alias of `grab-url`, kept for 2.x code — the identical module        |
| `grab-api.js`                     | The same core published without the loading icons — same `/full` and `/slim` subpaths. Install this **or** `grab-url`, never both |
...
| `grab-url-cli` · `grab-url` bin   | The transfer CLI and its primitives (`grab`, `g` are aliases), its own package |
...
installing `grab-url-cli` fetches automatically (`--no-ytdlp` opts out per-command, `--ytdlp` forces any URL
